...
def get_torvik_data(year, startyear, enddate, contype='All', venue='All'):
    ...
    # Skip empty rows (rows that don't have any data)
        ...
        team_name = re.sub(r'(\d+\s*seed,?\s*✅?)', '', team_name).strip()  # Remove seed-related text
...
AdamBomb["StRT+"] = AdamBomb.apply(StRTplus, axis =1)
# Strt+ is a threshold not exactly a rank.

AdamBomb["StRT+ Rank"] = AdamBomb["StRT+"].rank(ascending=True).astype(int)
...
# %%
# Avg Big 6 Rank (AB6R) [(TROE Rank)+ (ANH Rank)+ (AdjOE-AdjOE)/Tempo + mom Rank + "StRT+"]/6
def AB6R(row):
    return ((row['StREM Rank']+row['StROE Rank']+row['ANH Rank']+row['NonCon Rank']+row['Mom Rank']+row['StRT+'])/6)
