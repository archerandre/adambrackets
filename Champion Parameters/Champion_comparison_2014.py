...
def compare_to_champions(updated_file, champion_file):
    ...
    if not filtered_teams.empty:
        ...
        filtered_teams.to_csv("Teams_Within_Champion_Ranges_2014.csv", index=False)
        print("Filtered teams saved to 'Teams_Within_Champion_Ranges.csv'")
    else:
        print("No teams fit within the champion stat ranges.")

# Example usage
updated_file = "RPPF_UPDATED_RW4_Analyze_2014.csv"  # Update with actual path
